...
async function listBackups(guildId) { return all(`SELECT * FROM backups WHERE guild_id = ? ORDER BY created_at DESC`, [guildId]); }
async function getBackup(id, guildId) { return get(`SELECT * FROM backups WHERE id = ? AND guild_id = ?`, [id, guildId]); }
...
async function pruneBackups(guildId, maxBackups) {
  ...
  for (const backup of backups.slice(maxBackups)) await deleteBackup(backup.id, guildId);
}
