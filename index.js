...
function loadCommands(directory) {
    ...
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
        ...
        try {
            ...
            // Chỉ giữ MỘT command cho mỗi tên.
            if (client.commands.has(command.data.name)) {
                console.warn(`⚠️ Bỏ qua command trùng: /${command.data.name} (${filePath})`);
                continue;
            }
            ...
            console.error(`❌ Không thể load command: ${filePath}`, error);
        }
    }
}
...
async function registerCommands() {
    try {
        ...
        if (GUILD_ID) {
            // Chế độ test: chỉ đăng ký trong server GUILD_ID.
            // Đồng thời xóa global commands để tránh cùng một command
            // tồn tại ở cả global và guild.
            ...
            console.log("🧹 Đã xóa toàn bộ global command cũ.");
            return;
        }

        // Chế độ production: CHỈ dùng global commands.
        await rest.put(
        ...
        // Xóa command guild cũ ở tất cả server mà bot đang tham gia.
        // Không đăng ký guild command nữa.
        for (const guild of client.guilds.cache.values()) {
            try {
                ...
                console.log(`🧹 Đã xóa command guild cũ: ${guild.name}`);
            } catch (error) {
                console.error(`⚠️ Không thể xóa command guild ở ${guild.name}:`, error.message);
            }
        }
    } catch (error) {
...
client.once("clientReady", async () => {
    ...
    client.user.setPresence({
        activities: [{ name: "SkyRush Backup", type: ActivityType.Watching }],
        status: "online"
    });
...
client.on("interactionCreate", async interaction => {
    ...
    if (!command) {
        try {
            if (interaction.replied || interaction.deferred) {
                await interaction.editReply({ content: "❌ Command không tồn tại.", embeds: [], components: [] });
            } else if (interaction.isRepliable()) {
                await interaction.reply({ content: "❌ Command không tồn tại.", flags: 64 });
            }
        } catch {}
        return;
    }
    ...
    try {
        ...
        try {
            if (interaction.replied || interaction.deferred) {
                await interaction.editReply({ content: "❌ Đã xảy ra lỗi khi thực hiện command.", embeds: [], components: [] });
            } else if (interaction.isRepliable()) {
                await interaction.reply({ content: "❌ Đã xảy ra lỗi khi thực hiện command.", flags: 64 });
            }
        } catch {}
    }
});
