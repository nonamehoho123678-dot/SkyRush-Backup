...
if (!TOKEN) {
    console.error("❌ Thiếu DISCORD_TOKEN hoặc TOKEN trong file .env");
    process.exit(1);
}
...
function loadCommands(directory) {
    if (!fs.existsSync(directory)) {
        console.warn(`⚠️ Không tìm thấy thư mục: ${directory}`);
        return;
    }
    ...
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
        ...
        try {
            ...
            if (!command || !command.data || typeof command.data.name !== "string" || typeof command.execute !== "function") {
                console.warn(`⚠️ Bỏ qua file không phải command: ${filePath}`);
                continue;
            }

            if (client.commands.has(command.data.name)) {
                console.warn(`⚠️ Command trùng: /${command.data.name}`);
                continue;
            }
...
async function registerCommands() {
    try {
        ...
        if (GUILD_ID) {
            ...
            console.log(`🏠 Đã đăng ký ${commands.length} command cho server.`);
        } else {
...
function startAutoBackup() {
    ...
    if (!fs.existsSync(schedulerPath)) {
        console.log("⚠️ Không tìm thấy utils/autoBackup/scheduler.js");
        return;
    }
    ...
    try {
        ...
        if (typeof scheduler === "function") scheduler(client);
            ...
            console.warn("⚠️ scheduler.js không có function start/init.");
            return;
        }
...
client.on("interactionCreate", async interaction => {
    ...
    if (!command) {
        ...
        try {
            ...
            console.error("❌ Không thể gửi lỗi command:", error.message);
        }
        return;
    }
        ...
        try {
            if (interaction.replied || interaction.deferred) {
                await interaction.editReply({
                    content: "❌ Đã xảy ra lỗi khi thực hiện command.",
                    embeds: [],
                    components: []
                });
            } else if (interaction.isRepliable()) {
            ...
            console.error("❌ Không thể gửi lỗi Discord:", replyError.message);
        }
    }
});
